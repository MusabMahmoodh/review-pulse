 ...
 * 
 * This route receives the OAuth callback from Meta (Facebook) and forwards it to the backend server.
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(
          `/dashboard/settings?meta_error=${encodeURIComponent(error)}`,
          request.url
        )
      );
    }
    ...
    if (!code || !state) {
      return NextResponse.redirect(
        new URL(
          "/dashboard/settings?meta_error=missing_params",
          request.url
        )
      );
    }
    ...
    const backendCallbackUrl = new URL("/api/auth/meta/callback", backendUrl);
    ...
    if (!response.ok) {
      ...
      return NextResponse.redirect(
        new URL(
          `/dashboard/settings?meta_error=${encodeURIComponent(data.error || "unknown")}`,
          request.url
        )
      );
    }
    ...
    return NextResponse.redirect(
      new URL(
        "/dashboard/settings?meta_connected=true",
        request.url
      )
    );
  } catch (error) {
    ...
    return NextResponse.redirect(
      new URL(
        "/dashboard/settings?meta_error=unknown",
        request.url
      )
    );
  }
